...
def create_prob_index(root_dir, output_csv, top_range = 0, bottom_range = 0):
    ...
    if not os.path.exists(output_csv):
        try:
            with open(output_csv, 'x') as f:
                f.write("SNID,File_key,RA_Galaxy,Dec_Galaxy,Position_Angle,Major_Axis,Minor_Axis,Kron_Radius,Mag,Mag_Error,Flux,Flux_Error,ISOAREA_IMAGE,X_IMAGE,Y_IMAGE,CXX,CXY,CYY,OFFSET,RE,d_DLR,F_HOST_4")
    ...
    for index in range(top_range, bottom_range):
        ...
        try:
            ...
            skipped_index_dict.append(skipped_index_dict)


    dictionary = pd.DataFrame(data_dict)
    dictionary.to_csv(output_csv, mode = 'a', index=False, header = False)
    skipped = pd.DataFrame(skipped_index_dict)
    skipped.to_csv("skipped_index.csv", index = False)
